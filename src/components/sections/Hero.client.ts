...
if (!prefersReducedMotion()) {
  ...
  tl.from("[data-hero-top-l], [data-hero-top-r]", {
    y: 16,
    opacity: 0,
    duration: 0.8,
    stagger: 0.05,
  ...
  tl.from(
    "[data-hero-lead] .hero__sub, [data-hero-lead] .hero__badges li",
    { y: 16, opacity: 0, duration: 0.7, stagger: 0.06 },
    "-=0.6"
  );

  tl.from(
    "[data-hero-camera] .hero__camera-img",
    { y: 50, opacity: 0, scale: 0.94, rotate: -10, duration: 1.4 },
    "-=0.9"
  );

  tl.from(
    "[data-hero-camera] .hero__camera-tag",
  ...
  tl.from(
    "[data-hero-player], [data-hero-scroll]",
  ...
  mm.add("(min-width: 720px)", () => {
    ...
    gsap.to("[data-hero-camera] .hero__camera-img", {
      yPercent: 14,
      rotate: -1,
      ease: "none",
      scrollTrigger: {
  ...
  mm.add("(min-width: 720px) and (hover: hover) and (pointer: fine)", () => {
    ...
    const cameraWrap = document.querySelector<HTMLElement>("[data-hero-camera]");
    const crosshair = document.querySelector<HTMLElement>("[data-hero-crosshair]");
...
// Wire reel shortcut button to the Studio reel modal (if available)
const reelShortcut = document.querySelector<HTMLButtonElement>("[data-reel-shortcut]");
reelShortcut?.addEventListener("click", () => {
